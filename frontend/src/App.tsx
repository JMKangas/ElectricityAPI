...
function App() {
  ...
  const fetchAllLocations = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/weather_locations', {
  ...
  const fetchWeatherForecast = useCallback(
    async (idParam?: string, cityParam?: string) => {
      ...
      try {
        const url = `/api/weather?id=${encodeURIComponent(
          id
        ...
        const mapped: Forecast[] = Array.isArray(data.forecast)
          ? data.forecast.map((f: any) => ({
              ...
              summary:
                f.summary ??
                `Wind ${f.windSpeed ?? 'N/A'} m/s, Humidity ${
                  f.humidity ?? 'N/A'
                }%`,
      ...
      <main className="main-content">
        ...
        <WeatherSection
          ...
          fetchWeatherForecast={() =>
            fetchWeatherForecast(selectedId, selectedCity)
          }
          locations={locations}
